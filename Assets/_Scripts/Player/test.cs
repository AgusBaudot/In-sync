...
public class test : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        GatherInput();
        Look(); //Then we look towards input.
        Debug.Log($"Velocity before Move: {_rb.velocity}");
        if (_canMove) Move(); //Handle physics.
        //Debug.Log($"transform.forward: {transform.forward}, magnitude: {transform.forward.magnitude}");
        Debug.Log($"CanMove: {_canMove}, Input: {_input}, Magnitude: {_input.magnitude}, Velocity: {_rb.velocity}");
    }
    ...
    private void Move()
    {
        ...
        Debug.Log($"MoveDir: {moveDir}, Forward: {transform.forward}, Input: {_input}, Speed: {_speed}");
    }
